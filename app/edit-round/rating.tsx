...
export default function EditRatingScreen() {
  ...
  const [rating, setRating] = useState(() => {
    const r = parseFloat(params.rating as string);
    return isNaN(r) ? 7.5 : r;
  });
  ...
  const loadUserRatedCourses = async () => {
    try {
      ...
      // Fetch user's rounds with course info (excluding current course)
      const { data, error } = await supabase
        ...
        `)
        .eq('user_id', user.id)
        .neq('course_id', params.courseId as string)
        .not('rating', 'is', null);
  ...
  const uploadPhotos = async (photoUris: string[], userId: string): Promise<string[]> => {
    ...
    for (const uri of photoUris) {
      // Skip already uploaded photos (they start with http)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      handleError(err, 'Updating round');
      setIsSubmitting(false);
    }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Edit Rating</Text>
        <View style={{ width: 28 }} />
      </View>
          ...
          {loadingCourses ? (
            ...
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptyStateText}>
                This is your only rated course
              </Text>
            </View>
          ) : (
            <View style={styles.emptyState}>
